...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    /* The gate carries the same split lockup as the rest of the site, but
       written out longhand: this page renders before any stylesheet of ours
       is guaranteed to be reachable, so it cannot lean on the theme tokens.
       It is dark-only, so the dark palette is hard-coded. */
    .lockup {
      display: flex; flex-direction: column; align-items: center;
      gap: 0.32em; margin: 0 auto 2rem; font-size: clamp(58px, 17vw, 82px);
    }
    .lockup-mark {
      display: block; height: 1.44em; aspect-ratio: 598 / 471;
      background: url('/brand/mark.png') center / contain no-repeat;
      /* Near-black line-work on a dark ground: flip lightness, then put the
         hues back where invert() moved them. */
      filter: invert(1) hue-rotate(180deg);
    }
    /* Outer element frames the wordmark with the rules and carries no
       background of its own — the gradient has to measure across the glyph run
       alone, so it lives on .lockup-word inside. Putting it out here would
       stretch the ramp across the rules as well and drag every stop off the
       letter it was placed on. */
    .lockup-type {
      display: flex; align-items: center; gap: 0.55em;
      font-family: 'Cabinet Grotesk', 'Trebuchet MS', sans-serif;
      font-weight: 800; text-transform: uppercase; line-height: 1;
      font-size: 0.26em; white-space: nowrap;
      /* The mark's weight sits left of its bounding box: the solid leg is
         heavy while the circuit traces are thin, putting its centre of mass
         about 3.6% of its width left of geometric centre. Shifting the
         wordmark the same distance lines the two up by eye rather than by box. */
      transform: translateX(-0.25em);
    }
    /* Flanking rules, carried over from the original lockup. Coloured
       explicitly rather than with currentColor: the wordmark's colour is
       transparent so the gradient can show through it, and rules inheriting
       that would be invisible. */
    .lockup-type::before, .lockup-type::after {
      content: ''; display: block; flex: none;
      width: 1.5em; height: 0.047em; background: #d3ded8; opacity: 0.4;
    }
    .lockup-word {
      flex: none; letter-spacing: 0.2em;
      /* Pulls back the phantom half-space the trailing letter-spacing adds,
         so the wordmark centres on its glyphs rather than its box. */
      margin-right: -0.2em;
      color: #d3ded8;
    }
    @supports (background-clip: text) or (-webkit-background-clip: text) {
      .lockup-word {
        background-image: linear-gradient(90deg, #d3ded8 0%, #2ca0ff 52%, #ff8a2e 100%);
        background-image: linear-gradient(90deg in oklab, #d3ded8 0%, #2ca0ff 52%, #ff8a2e 100%);
...
<body>
  <main class="card">
    <span class="lockup" role="img" aria-label="Ahern AI">
      <span class="lockup-mark"></span>
      <span class="lockup-type"><span class="lockup-word">Ahern AI</span></span>
    </span>
    ...
    <!-- Nothing below the form on purpose. No phone number, no email, no
         contact form: the point of the gate is that nobody reaches out before
         the site is ready, and printing a way to get in touch here would undo
         it. The lockup at the top already says whose site this is, so a second
         wordmark down here was only repeating it. -->
  </main>
</body>
</html>`;
}
