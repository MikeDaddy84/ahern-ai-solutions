...
const CONTACT_PHONE = '(254) 693-4919';
const CONTACT_TEL = '+12546934919';

function aboutHtml() {
  return `
    <section class="container blog-hero about-intro">
      <div>
      ...
      <figure class="about-photo">
        <img src="/photos/mike-and-wife.jpg" alt="Mike Ahern and his wife smiling together outdoors" width="1200" height="900" fetchpriority="high" />
      </figure>
    </section>
    ...
    <section class="container post">
      <div class="post-body">
        ...
        <p>Then I moved to KCS as a technician. For several years I was the entire support staff &mdash; every ticket, every outage, every user in the company. I became VP/CTO in 2022, and left that role to build this.</p>
        ...
        <div class="about-family-photos">
          <figure class="about-photo">
            <img src="/photos/ahern-kids.jpg" alt="The Ahern kids laughing together outdoors in a black-and-white family portrait" width="1200" height="798" loading="lazy" decoding="async" />
          </figure>
          <figure class="about-photo">
            <img src="/photos/mike-and-wife-london.jpg" alt="Mike Ahern and his wife taking a selfie on a London street" width="900" height="1200" loading="lazy" decoding="async" />
          </figure>
        </div>
        ...
        <p>What it does have is my full attention. I left a VP/CTO job to do this; it is not a side project fitted around a day job, and you are not competing with someone else's payroll for my week.</p>
...
function privacyHtml() {
  return `
    <section class="container blog-hero">
      ...
      <p>Short version: information you send in an inquiry, records of our work together, and counts of page visits and service-tool activity. No advertising trackers, no third-party analytics, and nothing sold to anyone. Last updated September 2026.</p>
    </section>

    <section class="container post">
      <div class="post-body">
        <h2>When you fill in the contact form</h2>
        <p>The form stores the fields you send: <strong>your name, email address, optional business name, service interest, message, and any budget, timing, project details, or planning notes you include.</strong> Planning notes carried from a tool are visible and editable before sending. Alongside those it records the referring page's address without its query string and your browser's user-agent string, which identifies your browser and operating system.</p>
        <p>That information is used to reply to you and keep track of the conversation. The site also stores a request reference, a fingerprint to prevent duplicate submissions, and the status of the notification to my inbox. Your inquiry may be delivered through the configured email provider. It is not added to a mailing list or sold. If you would like it deleted, ask and it will be deleted.</p>

        <h2>When you just read the site</h2>
        <p>A first-party analytics beacon records <strong>which page was viewed, the referring page without its query string, your browser's user-agent, and the time.</strong> Separate service activity records contain only an event name, service category, and time. Events include choosing a service, completing a planner or demo, requesting a quote, and receiving confirmation that an inquiry was saved. These events do not contain your planner answers or contact fields.</p>
        ...
        <h2>Cookies</h2>
        <p><strong>No advertising or analytics cookies are set.</strong> While the site is behind its pre-launch password, an access cookie remembers that you have signed in. The portal also uses an essential sign-in cookie, with a session lasting up to 12 hours. Signing out ends that session.</p>
        ...
        <p>If you save a PC build for comparison, its specification and estimated price are stored in your browser tab's session storage. Use “Clear comparison” to remove it. Shared build links encode your selected answers. Requesting a quote sends the build summary with the contact form.</p>
        <p>Planning tools run in your browser. When you choose to discuss a plan, its summary is saved in that tab's session storage under <span class="mono">ahern-plan</span> and copied into the matching inquiry form. The form accepts that saved plan for up to 24 hours. It is removed after a successful inquiry or when the browser ends the tab's session. You can edit or remove the notes before submitting; only submitted notes go to the server.</p>

        <h2>When you use the portal and Satori</h2>
        <p>A provisioned portal account stores your name, email address, access label, password hash, and a reference to your personal workspace. Session records support sign-in and sign-out. Sign-in attempts are temporarily counted by email and IP address to limit repeated attempts. Portal pages do not run the public website’s analytics or load external fonts.</p>
        <p>A connected Satori workspace stores your task text, priorities, status, dates, backlog, and daily history. Workspaces share a database, and the server limits access to the records belonging to your workspace. If calendars are configured for your workspace, the server reads those feeds and may copy event details into your agenda. Ahern AI operates this service and its storage; “personal workspace” does not mean the data is hidden from the service operator.</p>
        <p>The portal preview contains sample tasks. Edits to those samples stay in that browser tab’s session storage and are not sent to a personal workspace. Closing the tab normally ends that preview session.</p>
        <h2>Third parties</h2>
        ...
        <p>The website runs on Render and inquiry data is stored in a hosted Turso database. When email delivery is enabled, the configured provider processes the notification and a copy reaches my inbox. These services support hosting, storage, and responding to your request.</p>
        <p>The private inquiry dashboard also stores sales stages, sale amounts, direct costs, delivery hours, and notes about project outcomes. These help manage inquiries and understand the cost of delivering services.</p>
        <p>When the portal is enabled, account, session, and workspace records are stored in new portal tables in Satori’s existing Turso database. Original Satori tables remain separate from these portal records. Local development and tests use local databases with sample data.</p>

        <h2>How long it is kept</h2>
        <p>Contact submissions and related project records are kept while there is a reason to &mdash; an ongoing conversation, a quote, an active project, or a warranty. Page-view and service-event records are retained for traffic and service-interest reporting; this site does not assign visitor identifiers to them.</p>
        <p>Portal account and task records remain until removed. Satori’s daily rollover archives older completed tasks without deleting them and removes daily history snapshots older than 90 days when rollover runs. Expired session records are cleaned up during later successful sign-ins. Contact Ahern AI to request account or workspace deletion.</p>
        ...
        <h2>Children</h2>
        <p>This site serves businesses and individual customers and is not directed at children under 13. No information is knowingly collected from them.</p>
