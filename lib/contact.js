...
const INTERESTS = new Set(['AI automation', 'Custom gaming PC', 'Professional workstation', 'Everyday or office PC', 'Business local AI system', 'Website or custom app', 'Networks & cabling', 'Not sure yet']);
function validate(body = {}) {
  ...
  const extras = [['budget', 'Project budget', 100], ['timeline', 'Timing', 100], ['projectDetails', 'Project details', 600], ['context', 'Planning notes', 3000]];
  for (const [key, label, max] of extras) {
    if (body[key] != null && (typeof body[key] !== 'string' || body[key].length > max)) throw new Error('Please check your project details.');
    const value = (body[key] || '').trim();
    if (value) lead.message += `${lead.message ? '\n\n' : ''}${label}: ${value}`;
  }
