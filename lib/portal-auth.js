...
function createStore(client) {
  ...
  return {
    client, prepare,
    async setupAccount({email, name, workspace, role='member'}) {
      ...
      // Create only: never reset an existing account or replace its workspace.
      await client.batch([
        {sql:'INSERT INTO portal_users(id,email,name,password_hash,workspace_key,role,disabled) VALUES(?,?,?,?,?,?,1)',args:[id,email.trim().toLowerCase(),name.trim(),passwordHash,workspace,role]},
        {sql:'INSERT INTO portal_account_setup(token_hash,user_id,expires_at) VALUES(?,?,?)',args:[digest(value),id,Date.now()+24*60*60*1000]}
      ],'write');
      return value;
    },
    async finishSetup(value, password) {
      if (!/^[a-f0-9]{64}$/.test(value || '')) return false;
      const passwordHash=await hashPassword(password);
      await prepare();
      const tx=await client.transaction('write');
      try {
        const result=await tx.execute({sql:'UPDATE portal_users SET password_hash=?,disabled=0 WHERE disabled=1 AND id IN (SELECT user_id FROM portal_account_setup WHERE token_hash=? AND expires_at>?) RETURNING id',args:[passwordHash,digest(value),Date.now()]});
        if (!result.rows.length) {await tx.rollback();return false;}
        await tx.execute({sql:'DELETE FROM portal_account_setup WHERE token_hash=?',args:[digest(value)]});
